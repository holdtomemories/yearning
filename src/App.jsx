...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    filteredPins.forEach(p => {
      ...
      const html = `<div style="position:relative;width:${w}px;height:${h}px;transform:translateY(-${h/2 - w/2}px)"><svg width="${w}" height="${h}" viewBox="0 0 24 32" style="position:absolute;inset:0;filter:drop-shadow(0 0 ${sel ? 10 : 6}px ${mc}cc) drop-shadow(0 2px 3px rgba(0,0,0,0.4));transition:all 0.2s" xmlns="http://www.w3.org/2000/svg"><path d="M12 1 C5.5 1 1 5.5 1 11 C1 18 12 31 12 31 C12 31 23 18 23 11 C23 5.5 18.5 1 12 1 Z" fill="${mc}" stroke="${stroke}" stroke-width="1.6" stroke-linejoin="round"/><circle cx="12" cy="11" r="3.6" fill="${stroke}" opacity="0.92"/></svg>${sel ? `<div style="position:absolute;left:50%;bottom:-3px;transform:translateX(-50%);width:${w*.9}px;height:${w*.9}px;border-radius:50%;border:1.5px solid ${mc}88;animation:pulseRing 1.4s ease-out infinite"></div>` : ""}</div>`;
